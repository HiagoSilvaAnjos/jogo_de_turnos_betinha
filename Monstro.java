...
public abstract class Monstro extends Player {
    ...
    public ResultadoAtaque realizarAtaque(Player alvo) {
        ...
        if (roll <= chanceCritico) {
            // Dano crítico do monstro é 1.8x a força de ataque, menos a defesa do alvo.
            // Representa um ataque mais forte, mas ligeiramente inferior ao dos heróis (que usam 2x).
            double danoCritico = forcaAtaque * 1.8 - alvo.getDefesa();

            // Garante no mínimo 1 de dano mesmo que a defesa seja alta,
            // e impede que o HP do alvo fique negativo.
            alvo.setHP(Math.max(0, alvo.getHP() - Math.max(1, danoCritico)));

            // Retorna CRÍTICO como resultado do ataque.
